...
public class JobCommand {
    ...
    private static int setEXP(CommandSourceStack source, Player target, Jobs job, int exp) {
        if (source.getEntity() instanceof Player player) {
            if (JobGetters.getJobLevel(target, job) == 0) {
                ChatHandler.sendMessage(player, JobsPlus.translatable("error.exp.level_is_0", ChatColor.boldDarkRed(), ChatColor.red()).getString());
                return 1;
            }
            int maxJobEXP = LevelHandler.calcExp(JobGetters.getJobLevel(target, job)) - 1;
            if (exp > maxJobEXP) {
                ChatHandler.sendMessage(player, JobsPlus.translatable("error.exp.cannot_be_higher_than_max", ChatColor.boldDarkRed(), ChatColor.red(), maxJobEXP).getString());
                return 1;
            }
            if (exp < 0) {
                ChatHandler.sendMessage(player, JobsPlus.translatable("error.exp.cannot_be_negative", ChatColor.boldDarkRed(), ChatColor.red()).getString());
    ...
    private static int setCoins(CommandSourceStack source, Player target, int coins) {
        if (source.getEntity() instanceof Player player) {
            if (coins < 0) {
                ChatHandler.sendMessage(player, JobsPlus.translatable("error.coins.cannot_be_negative", ChatColor.boldDarkRed(), ChatColor.red()).getString());
                return 1;
            }
    ...
    private static int setDisplay(CommandSourceStack source, Player target, Jobs job) {
        if (source.getEntity() instanceof Player player) {
            if (JobGetters.jobIsEnabled(target, job)) {
                ...
                ChatHandler.sendMessage(player, JobsPlus.translatable("error.job.target_not_performing", ChatColor.boldDarkRed(), ChatColor.red(), target.getScoreboardName()).getString());
            }
        }
        return 1;
    }
