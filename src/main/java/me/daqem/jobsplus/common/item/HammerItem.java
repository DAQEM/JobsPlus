...
public class HammerItem extends JobsPlusItem.PickAxe {
    ...
    public boolean canAttackBlock(@NotNull BlockState state, @NotNull Level level, @NotNull BlockPos pos, @NotNull Player player) {
        if (!level.isClientSide) {
            if (JobGetters.jobIsEnabled(player, Jobs.MINER)) {
                ...
                if (JobGetters.getJobLevel(player, Jobs.MINER) >= getRequiredLevel()) {
    ...
    public void appendHoverText(@NotNull ItemStack stack, @Nullable Level worldIn, @NotNull List<Component> tooltip, @NotNull TooltipFlag flagIn) {
        ...
        if (Screen.hasShiftDown()) {
            ...
            tooltip.add(Component.literal(ChatColor.green() + "Job: " + ChatColor.reset() + "Miner"));
            ...
            tooltip.add(Component.literal(ChatColor.green() + "Item Level: " + ChatColor.reset() + Objects.requireNonNull(stack.getItem().getDescriptionId()).replace("item.jobsplus.miners_hammer_level_", "")));
