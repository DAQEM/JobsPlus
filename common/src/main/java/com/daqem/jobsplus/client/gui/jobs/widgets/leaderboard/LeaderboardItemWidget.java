...
public class LeaderboardItemWidget extends CustomButtonWidget {
    ...
    protected void renderContents(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        int headSize = 12;
        PlayerFaceRenderer.draw(guiGraphics, skinLocation, headX, headY, headSize, true, false, -1);

        String levelString = String.valueOf(player.getLevel());
        int levelWidth = minecraft.font.width(levelString);
        guiGraphics.drawString(minecraft.font, levelString, this.getX() + this.getWidth() - 3 - levelWidth, this.getY() + 3, getColor(player), false);
        ...
        this.nameComponent.renderBase(guiGraphics, mouseX, mouseY, partialTick, this.getWidth(), this.getHeight());

        guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, JobsPlus.getId("jobs/exp_bar"), getX() + 20, getY() + 13, 76, 3);
        double expPercentage = (double) player.getExperience() / (double) Job.getExperienceToLevelUp(player.getLevel()) * 100;
