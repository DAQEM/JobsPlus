...
public class BackpackItem extends Item {
    ...
    public InteractionResultHolder<ItemStack> use(Level level, @NotNull Player player, @Nonnull InteractionHand hand) {
        ...
        if (!backpack.is(ModItems.ENDER_BACKPACK.get())) {
            NetworkHooks.openGui(((ServerPlayer) player), new SimpleMenuProvider((windowId, playerInventory, playerEntity) -> new BackpackMenu(windowId, playerInventory, uuid, data.getTier(), data.getHandler()), backpack.getHoverName()), (buffer -> buffer.writeUUID(uuid).writeInt(data.getTier().ordinal())));
        } else {
            NetworkHooks.openGui(((ServerPlayer) player), new SimpleMenuProvider((containerId, inventory, player1) ->
