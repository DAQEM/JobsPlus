...
import me.daqem.jobsplus.init.ModContainers;
...
public class BackpackContainer extends AbstractContainerMenu {
    ...
    public BackpackContainer(final int windowId, final Inventory playerInventory, UUID uuidIn, Backpack tierIn, IItemHandler handler) {
        super(ModContainers.BACKPACK_CONTAINER.get(), windowId);
    ...
    public static BackpackContainer fromNetwork(final int windowId, final Inventory playerInventory, FriendlyByteBuf data) {
        UUID uuidIn = data.readUUID();
        Backpack tier = Backpack.values()[data.readInt()];
        return new BackpackContainer(windowId, playerInventory, uuidIn, tier, new ItemStackHandler(tier.slots));
    }
    ...
    private void addMySlots() {
        ...
        for (int row = 0; row < rows; row++) {
            for (int col = 0; col < cols; col++) {
                ...
                this.addSlot(new BackpackContainerSlot(this.handler, slot_index, x + 1, y + 1));
                slot_index++;
